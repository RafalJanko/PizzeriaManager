...
class BaseUserModelTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.user = User(
            first_name="jan", last_name="kowalski", password="asd", email="asd@asd.com"
        )
        cls.user.save()
...
class BaseOrderItemModelTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.order_item = OrderItem(
            price=15.50, size=30, order_id=1, pizza_name="WhatevsPizza", quantity=4
        )
        cls.order_item.save()
        cls.customer = Customer(
            user_id=1,
...
class ToppingViewsTests(TestCase):
    def test_topping_update_view(self):
        ...
        response = self.client.post(
            reverse("update_topping", kwargs={"pk": topping.pk}), {"name": "test"}
        )
...
class PizzaViewsTests(TestCase):
    def test_pizza_update_view(self):
        pizza = PizzaMenuItem.objects.create(
            id=1, name="test", image="temp", img="temp", price=15.0, size=30
        )
        response = self.client.post(
            reverse("update_pizza_details", kwargs={"pk": pizza.pk}), {"name": "test"}
        )
...
class ShiftViewsTests(TestCase):
    def test_shift_update_view(self):
        ...
        shift = Shift.objects.create(
            id=1, date=date(2022, 10, 15), user_id=1, shift="12:00 - 20:00"
        )
        response = self.client.post(
    ...
    def test_shift_delete_view(self):
        ...
        resp = self.client.post(
            reverse("delete_shift_manager", kwargs={"pk": shift.pk})
        )
        self.assertEqual(resp.status_code, 302)
...
class OrderViewsTests(TestCase):
    def test_order_update_view(self):
        ...
        order = Order.objects.create(
            id=1, total=60.0, payment_method="cash", customer_id=1, address="asd"
        )
        response = self.client.post(
    ...
    def test_order_delete_view(self):
        ...
        resp = self.client.post(
            reverse("delete_orders_manager", kwargs={"pk": order.id})
        )
        self.assertEqual(resp.status_code, 302)
...
class DaysOffViewsTests(TestCase):
    def test_daysoff_update_view(self):
        ...
        response = self.client.post(
            reverse("update_leave", kwargs={"pk": dayoff.id}), {"status": "Accepted"}
        )
...
class UserViewsTests(TestCase):
    def test_user_update_view(self):
        ...
        response = self.client.post(
            reverse("UpdateUser", kwargs={"pk": user.id}), {"email": "test@test.com"}
        )
