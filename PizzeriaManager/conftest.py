...
def shift():
    return Shift.objects.create(
        date=date(2022, 3, 12), user_id=1, shift="12:00 - 20:00"
    )
...
def shifts(user):
    lst = []
    for i in range(10):
        x = Shift.objects.create(user_id=i, date=2022 - 10 - 12, shift="12:00 - 20:00")
        lst.append(x)
    return lst
...
def pizza(ingredient):
    PizzaMenuItem.objects.create(
        name="test", category="Pizza", img="test", image="temp", price=15, size="30"
    )
...
def pizzas():
    lst = []
    for i in range(3):
        x = PizzaMenuItem.objects.create(
            id=i,
            name=f"test{i}",
            category="Pizza",
            img="test",
